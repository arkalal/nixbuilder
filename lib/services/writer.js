...
- If a PROJECT CONTEXT is provided (existing files list and/or content), treat the task as an incremental edit to the EXISTING project.
- Do NOT recreate base files unless strictly required; only output files you MODIFY or ADD.
- Keep architecture and prior code unless the user asks to change it.
- Prefer minimal diffs across files, but always output full file contents between <file> tags.
...
export async function generateCode(userPrompt, options = {}) {
  ...
  // Build context for iterative edits if provided
